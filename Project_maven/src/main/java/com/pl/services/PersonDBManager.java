...
public class PersonDBManager {
	...
	public PersonDBManager() 
	{
		...
		try 
		{
			...
			{
				stmt.executeUpdate("CREATE TABLE person(id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,imie varchar(20),nazwisko varchar(20))");
			}

			addPersonStmt = conn.prepareStatement("INSERT INTO person (imie, nazwisko) VALUES (?, ?)");
			
			getPersonStmt = conn.prepareStatement("SELECT * FROM person");

			deleteAllPersonStmt = conn.prepareStatement("DELETE FROM person");

			findPersonStmt = conn.prepareStatement("SELECT id FROM person WHERE imie = ?");
	...
	public void addPerson(Person person) 
	{
		try 
		{
			addPersonStmt.setString(1, person.getImie());
			addPersonStmt.setString(2, person.getNazwisko());
	...
	public List<Person> getAllPersons() 
	{
		...
		try 
		{
			...
			while (rs.next()) 
			{
				persons.add(new Person(rs.getString("imie"),rs.getString("nazwisko"), null));
			}
